...
import DATA_FORMATION from './../../data/data_formation.js'

function Formation ({formation, isSideMenuShown}){
    return <div className="formation">
        <div className="formation-header">
            <p className="formation-date">{formation.date}</p>
        </div>
        <div className={isSideMenuShown ? "formation-corps-img sidemenuopen" : "formation-corps-img"}>
        ...
        <div className="formation-corps">
            <h4>
                <span className={isSideMenuShown ? "formation-name sidemenuopen" : "formation-name"}>{formation.name}</span>                    
                <br/>
                <a 
...
class FormationList extends Component {
    ...
    render() { 
        const formationList = DATA_FORMATION.map((formation,key)=>{
            return <Formation
